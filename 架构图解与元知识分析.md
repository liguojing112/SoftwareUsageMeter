...
```mermaid
flowchart TB
    subgraph MainThread["主线程 (main.py)"]
        App["Application<br/>状态管理"]
        Overlay["PaymentOverlay<br/>收费弹窗"]
        WaitOverlay["ExportWaitOverlay<br/>等待遮罩"]
        StartupHint["StartupHintDialog<br/>启动提示"]
        Tray["TrayIconManager<br/>托盘图标"]
    end

    subgraph MonitorThread["监控线程 (process_monitor.py)"]
        Monitor["ProcessMonitor"]
        Detect["导出检测<br/>热区/窗口/OCR"]
        OCR["Windows OCR<br/>张数识别"]
    end

    subgraph Data["数据层"]
        Config["ConfigManager<br/>config.json"]
        Timer["TimerManager<br/>计时器"]
    end

    subgraph External["外部"]
        PixCake["PixCake.exe<br/>像素蛋糕"]
    end

    %% 启动流程
    Config --> App
    App --> StartupHint
    StartupHint -->|"知道了"| Monitor
    Monitor -->|"启动"| Detect
    ...
    %% 信号连接
    Monitor -->|"process_started"| App
    Monitor -->|"export_detected"| App
    Monitor -->|"export_button_pre_clicked"| App
    Monitor -->|"export_cancelled"| App
    App -->|"set_export_state_hold"| Monitor

    %% UI流程
    App -->|"show_wait"| WaitOverlay
    WaitOverlay -->|"5秒后"| Overlay
    App -->|"show_payment"| Overlay
    OCR -->|"张数"| Overlay
    Overlay -->|"payment_completed"| App
    ...
    style MonitorThread fill:#fff3e0
...
| 信号 | 发射者 | 接收者 | 触发时机 |
|------|--------|--------|----------|
| process_started | ProcessMonitor | Application | 检测到PixCake启动 |
| process_stopped | ProcessMonitor | Application | PixCake退出 |
| export_detected | ProcessMonitor | Application | 检测到导出行为 |
| export_button_pre_clicked | ProcessMonitor | Application | 点击导出按钮瞬间 |
| export_cancelled | ProcessMonitor | Application | 导出窗口关闭 |
| payment_completed | PaymentOverlay | Application | 确认收款 |
...
flowchart TD
    ...
    E --> F[显示启动说明]
    F -->|用户点击知道了| G[启动 ProcessMonitor 线程]
    ...
    K -->|主链路：点击导出按钮| K1[快速热区守卫检测：颜色+位置验证]
    K1 --> K2[设置 _is_exporting=True，传递参数]
    K2 --> M[显示导出等待遮罩，5秒倒计时]

    K -->|副链路：导出至创意| L{是导出至创意中转页？}
    L -->|是| L1[设置创意上下文保护，60秒超时]
    L1 --> L2[用户进入创意编辑页]
    L2 --> L3{点击导出至本地？}
    L3 -->|是| M
    L3 -->|否：提示或其他操作| L2
    ...
    M --> N1{5秒内取消？}
    N1 -->|是，取消| N2[恢复进程，恢复计时]
    N2 --> J
    N1 -->|否，确认| O[显示收费弹窗]

    O --> P{OCR识别张数}
    P -->|成功且张数>0| Q[显示费用明细]
    P -->|失败或张数=0| U[切换手动输入模式，显示输入框]
    U --> U1[管理员手动填写张数]
    U1 --> Q
