...
sequenceDiagram
    actor 顾客
    actor 店员
    participant SoftMeter as SoftwareUsageMeter<br/>(main.py)
    participant Monitor as ProcessMonitor<br/>(process_monitor.py)
    participant Timer as TimerManager<br/>(timer_manager.py)
    participant Overlay as PaymentOverlay<br/>(payment_overlay.py)
    participant Config as ConfigManager<br/>(config_manager.py)
    participant PixCake as PixCake.exe

    Note over SoftMeter: === 初始化阶段 ===
    SoftMeter->>Config: load() 加载配置
    Config-->>SoftMeter: config.json 数据
    SoftMeter->>Monitor: start() 启动监控线程
    SoftMeter->>SoftMeter: _prewarm_export_wait_overlay()
    SoftMeter->>SoftMeter: _start_fast_export_click_timer()
    Note right of SoftMeter: 20ms 轻量热区轮询<br/>热区范围：窗口35%宽度×18%高度

    Note over PixCake,SoftMeter: === 顾客打开像素蛋糕 ===
    顾客->>PixCake: 启动应用程序
    Monitor->>PixCake: find_pid_by_name("PixCake.exe")
    Monitor->>Monitor: is_valid_target_main_window()
    Note right of Monitor: 验证主窗口可见 &gt;500x350
    Monitor-->>SoftMeter: 信号: process_started
    SoftMeter->>Timer: start() 开始计时
    Timer-->>SoftMeter: 信号: tick(每秒) / minute_tick(每分钟)
    SoftMeter->>SoftMeter: 托盘图标变绿

    Note over PixCake,SoftMeter: === 主链路：点击导出按钮 ===
    顾客->>PixCake: 点击右上角"导出"按钮
    SoftMeter->>SoftMeter: 快速热区守卫检测
    Note right of SoftMeter: 颜色检测：红≥140 绿≥100 蓝≤120<br/>热区范围：窗口右上角35%×18%
    SoftMeter->>SoftMeter: 设置 _is_exporting=True
    SoftMeter->>SoftMeter: 获取计时时长和费率
    SoftMeter->>SoftMeter: _show_export_wait_overlay(minutes, rate)
    Note right of SoftMeter: 5秒全屏等待遮罩<br/>同时设置参数，防止重复触发
    SoftMeter->>Monitor: set_export_state_hold(True)

    Note over PixCake,SoftMeter: === 副链路：导出至创意 ===
    顾客->>PixCake: 点击"导出至创意"
    Monitor->>Monitor: looks_like_creative_transfer_layout()
    Monitor->>Monitor: 设置创意上下文保护(60秒)
    Note right of Monitor: _creative_transfer_context_active=True
    顾客->>PixCake: 进入创意编辑页
    PixCake->>PixCake: 弹出"成功导入X张图片"提示
    Monitor->>Monitor: 检测到窗口变化
    Monitor->>Monitor: 检查创意上下文保护
    Note right of Monitor: 保护生效，不触发收费
    顾客->>PixCake: 点击"导出至本地"
    SoftMeter->>SoftMeter: 触发导出等待遮罩

    Note over Overlay: === 收费阶段 ===
    SoftMeter->>Overlay: show_payment(计时, 张数, 单价)
    Overlay->>Overlay: calculate_payment_details()
    Note right of Overlay: 总费用=时长×计时单价+张数×导出单价

    Monitor->>PixCake: capture_window_image()
    Monitor->>Monitor: run_windows_ocr() 识别张数
    alt OCR 识别成功且张数>0
        Monitor-->>SoftMeter: 导出张数 N
        SoftMeter->>Overlay: update_display(张数=N)
    else OCR 识别失败或张数=0
        Monitor-->>SoftMeter: 识别失败或张数=0
        SoftMeter->>Overlay: set_manual_export_count_required(True)
        Note right of Overlay: 切换手动输入模式<br/>确认按钮禁用直到张数>0
    end

    Overlay->>PixCake: _lock_windows() 锁定窗口
    顾客->>Overlay: 扫微信/支付宝二维码付款

    Note over 店员,Overlay: === 确认收款 ===
    店员->>Overlay: 点击"已付款·确认收款"
    Overlay->>Overlay: _emit_payment_completed()
    Note right of Overlay: 验证张数>0(手动模式)
    Overlay-->>SoftMeter: 信号: payment_completed

    Note over SoftMeter,PixCake: === 恢复阶段 ===
    SoftMeter->>SoftMeter: _finish_export_cycle()
    SoftMeter->>Overlay: close_payment() 关闭弹窗
    Overlay->>PixCake: _unlock_windows() 解锁窗口
    SoftMeter->>Monitor: restore_target_interaction()
    Monitor->>PixCake: resume_processes() 恢复进程
    SoftMeter->>Timer: stop_and_report() → reset() → start()
    Note right of Timer: 清零旧计时<br/>开始新周期
    SoftMeter->>SoftMeter: POST_PAYMENT_CLEAR_DEBOUNCE(尾迹保护)

    Note over PixCake: 顾客继续修图，进入下一个导出周期
```
...
你的项目使用 PyQt5 信号/槽机制作为模块间通信的唯一方式。这对应 **Observer Pattern（观察者模式）**：
